...
local placeDock =
    function(c, m) awful.placement.bottom(c, {margins = dpi(m)}) end
...
local dock = function(s)
    ...
    local function check_for_dock_hide()
        ...
        if awful.screen.focused() == mouse.screen then
            ...
            for _, c in ipairs(awful.screen.focused().selected_tag:clients()) do
                if c.minimized then minimized = true end
            ...
            if minimized then dock.visible = true end
        else
            dock.visible = false
        end
    end
    ...
    local dockHide = gears.timer {
        ...
        autostart = true,
        call_now = true,
        callback = function() check_for_dock_hide() end
    }
    ...
    placeDock(hotpop, 0)
    hotpop:setup{widget = wibox.container.margin, margins = 10, layout = layout}
    ...
    local createDockIndicators = function(data)
        ...
        for _, v in ipairs(clients) do
            ...
            if v == client.focus then
                ...
                click = function() v.minimized = true end
            ...
            local widget = wibox.widget {
                ...
                forced_width = dpi(20),
                widget = wibox.container.background,
                buttons = {awful.button({}, 1, function() click() end)}
            }
        ...
        return wibox.widget {
            {
                {indicators, spacing = dpi(4), layout = layout},
                widget = wibox.container.place,
                halign = 'center'
            },
            forced_height = 4,
    ...
    local createDockElement = function(data)
        ...
        -- 
        local customIcons = {
            {name = "st", convert = "utilities-terminal", command = "st"},
            {name = "steam", convert = "steam", command = "steam-native"},
            {name = "nemo", convert = "system-file-manager", command = "nemo"},
            {
                name = "prismlauncher",
                convert = "org.prismlauncher.PrismLauncher",
    ...
    local createDockElements = function()
        ...
        local clients = mouse.screen.selected_tag and
                            mouse.screen.selected_tag:clients() or {}

        -- making some pinned apps
        local metadata = {
            {count = 0, id = 1, clients = {}, name = "st", class = "st"},
            {
                count = 0,
                id = 2,
                clients = {},
        ...
        local classes = {
            "st", "firefox", "vscodium", "prismlauncher", "prismlauncher"
        }
        local dockElements = wibox.widget {layout = layout, spacing = dpi(8)}
    ...
    local refresh = function()
        ...
        dock:setup{
            {
                {createDockElements(), layout = layout},
                widget = wibox.container.margin,
                margins = {top = 10, bottom = 7, left = 10, right = 10}
            },
            widget = wibox.container.background,
...
screen.connect_signal('request::desktop_decoration', function(s) dock(s) end)
